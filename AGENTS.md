...
4. For every new stack directory, create a `name` file so Unraid Compose Manager / Dockge can identify the stack.
5. For every stack directory, symlink the shared `.env` file into the stack directory:
...
6. If the stack should be managed through the helper targets, add it to [`docker/Makefile`](/home/jakobe/code/homelab/docker/Makefile).
